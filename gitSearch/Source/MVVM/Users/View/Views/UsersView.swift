...
class UsersView: UIView {
    ...
    lazy var subtitleView: UILabel = {
        let label = UILabel()
        label.text = "Choose a language clicking on image below to find a Users."
    ...
    required init?(coder: NSCoder) {
        fatalError("UsersView Usuported")
    }
...
extension UsersView: UsersViewModelDelegate {
    func didSelectLanguage(_ linguage: String) {
            spinner.startAnimating()
            delegate?.goToUsersList(linguage)
    }
}
